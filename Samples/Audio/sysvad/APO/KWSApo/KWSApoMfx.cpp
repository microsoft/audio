//
// KWSApoMFX.cpp -- Copyright (c) Microsoft Corporation. All rights reserved.
//
// Description:
//
//  Implementation of CKWSApoMFX
//
...
const AVRT_DATA CRegAPOProperties<1> CKWSApoMFX::sm_RegProperties(
    __uuidof(KWSApoMFX),                           // clsid of this APO
    L"CKWSApoMFX",                                 // friendly name of this APO
    L"Copyright (c) Microsoft Corporation",         // copyright info
    1,                                              // major version #
    0,                                              // minor version #
    __uuidof(IKWSApoMFX),                          // iid of primary interface
...
//
STDMETHODIMP_(void) CKWSApoMFX::APOProcess(
...
STDMETHODIMP CKWSApoMFX::GetLatency(HNSTIME* pTime)  
{  
    ASSERT_NONREALTIME();  
    HRESULT hr = S_OK;  
...
//                                          this APO.
STDMETHODIMP CKWSApoMFX::LockForProcess(UINT32 u32NumInputConnections,
...
HRESULT CKWSApoMFX::Initialize(UINT32 cbDataSize, BYTE* pbyData)
{
    HRESULT                     hr = S_OK;
    ...
    if (cbDataSize == sizeof(APOInitSystemEffects3))
    {
        ...
        {
            m_apoLoggingService->ApoLog(APO_LOG_LEVEL_INFO, L"CKWSApoMFX::Initialize called with APOInitSystemEffects3.");
        }        
    }
...
//
//
STDMETHODIMP CKWSApoMFX::GetEffectsList(_Outptr_result_buffer_maybenull_(*pcEffects) LPGUID *ppEffectsIds, _Out_ UINT *pcEffects, _In_ HANDLE)
{
    ...
    *pcEffects = 0;
    ...
    static const GUID effectsList[] = {AUDIO_EFFECT_TYPE_ACOUSTIC_ECHO_CANCELLATION, AUDIO_EFFECT_TYPE_NOISE_SUPPRESSION, AUDIO_EFFECT_TYPE_AUTOMATIC_GAIN_CONTROL};
...
//
//
STDMETHODIMP CKWSApoMFX::IsInputFormatSupported(IAudioMediaType *pOutputFormat, IAudioMediaType *pRequestedInputFormat, IAudioMediaType **ppSupportedInputFormat)
{
    ASSERT_NONREALTIME();
...
//
//
STDMETHODIMP CKWSApoMFX::IsOutputFormatSupported(IAudioMediaType *pInputFormat, IAudioMediaType *pRequestedOutputFormat, IAudioMediaType **ppSupportedOutputFormat)
{
    ASSERT_NONREALTIME();
    bool formatChanged = false;
    HRESULT hResult;
...
//
//
STDMETHODIMP CKWSApoMFX::GetInputChannelCount(UINT32 *pu32ChannelCount)
{
    ASSERT_NONREALTIME();
...
STDMETHODIMP CKWSApoMFX::GetControllableSystemEffectsList(_Outptr_result_buffer_maybenull_(*numEffects) AUDIO_SYSTEMEFFECT** effects, _Out_ UINT* numEffects, _In_opt_ HANDLE)
{
    ...
    *numEffects = 0;
    ...
    static const AUDIO_SYSTEMEFFECT effectsList[] = {{AUDIO_EFFECT_TYPE_ACOUSTIC_ECHO_CANCELLATION, FALSE, AUDIO_SYSTEMEFFECT_STATE_ON},
                                                     {AUDIO_EFFECT_TYPE_NOISE_SUPPRESSION,          FALSE, AUDIO_SYSTEMEFFECT_STATE_ON},
                                                     {AUDIO_EFFECT_TYPE_AUTOMATIC_GAIN_CONTROL,     FALSE, AUDIO_SYSTEMEFFECT_STATE_ON}};
