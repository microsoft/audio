...
{
    return (GetPinType(nPinId) == SystemCapturePin);
}

#pragma code_seg()
BOOL CMiniportWaveRT::IsCellularBiDiCapturePin(ULONG nPinId)
{
    return (GetPinType(nPinId) == TelephonyBidiPin);
}

#pragma code_seg()
BOOL CMiniportWaveRT::IsSystemRenderPin(ULONG nPinId)
{
    return (GetPinType(nPinId) == SystemRenderPin);
}

#pragma code_seg()
BOOL CMiniportWaveRT::IsLoopbackPin(ULONG nPinId)
{
    return (GetPinType(nPinId) == RenderLoopbackPin);
}

#pragma code_seg()
BOOL CMiniportWaveRT::IsOffloadPin(ULONG nPinId)
{
    return (GetPinType(nPinId) == OffloadRenderPin);
}

#pragma code_seg()
BOOL CMiniportWaveRT::IsBridgePin(ULONG nPinId)
{
    return (GetPinType(nPinId) == BridgePin);
}

#pragma code_seg()
BOOL CMiniportWaveRT::IsKeywordDetectorPin(ULONG nPinId)
{
    return (GetPinType(nPinId) == KeywordCapturePin);
}

#pragma code_seg()
PINTYPE CMiniportWaveRT::GetPinType(ULONG nPinId)
{
    PINTYPE pinType = NoPin;

    AcquireFormatsAndModesLock();

    if (nPinId < m_DeviceFormatsAndModesCount)
    {
        pinType = m_DeviceFormatsAndModes[nPinId].PinType;
    }
...
--*/
{
    ...
    KSATTRIBUTE_AUDIOSIGNALPROCESSING_MODE attributeMode = {0};
    NTSTATUS ntStatus = STATUS_SUCCESS;

    *_pSignalProcessingMode = AUDIO_SIGNALPROCESSINGMODE_DEFAULT;

    ntStatus = GetAttributeFromAttributeList(_pAttributes,
                                             _Size,
                                             &KSATTRIBUTEID_AUDIOSIGNALPROCESSING_MODE,
                                             (BYTE*)&attributeMode,
                                             sizeof(attributeMode));
    if (NT_SUCCESS(ntStatus))
    {
        *_pSignalProcessingMode = attributeMode.SignalProcessingMode;
    }
    else if (ntStatus == STATUS_NOT_FOUND)
    {
        // Use default-mode specified above.
        ntStatus = STATUS_SUCCESS;
    }
...
#pragma code_seg("PAGE")
NTSTATUS
CMiniportWaveRT::GetAttributeFromAttributeList
(
    _In_ const KSMULTIPLE_ITEM *_pAttributes,
    _In_ size_t         _AttributesSize,
    _In_ const GUID *   _pAttribute,
    _Out_writes_bytes_(_ValueSize) BYTE * _pValue,
    _In_ ULONG          _ValueSize
)
/*++
...
Arguments:
  ...
  _AttributesSize - count of bytes in the buffer pointed to by _pAttributes.
    The routine verifies sufficient buffer size while processing the attributes.

  _Attribute - pointer to the attribute GUID to find.

  _pValue - returns the attribute data.

  _pValueSize - size of the pValue buffer to hold the attribute data.
...
--*/
{
    ...
    DPF_ENTER(("[CMiniportWaveRT::GetAttributeFromAttributeList]"));

    size_t cbRemaining = _AttributesSize;
    ...
    for (ULONG i = 0; i < _pAttributes->Count; i++)
    {
        ...
        if (attributeHeader->Attribute == *_pAttribute)
        {
            if (_ValueSize < attributeHeader->Size)
            {
                return STATUS_INVALID_PARAMETER;
            }

            RtlCopyMemory(_pValue, attributeHeader, attributeHeader->Size);
            return STATUS_SUCCESS;
        }
        else
        {
            ULONGLONG cbAttribute = attributeHeader->Size;
            
            if (cbRemaining != attributeHeader->Size)
            {
                // Adjust pointer and buffer size to next attribute (QWORD aligned)
                cbAttribute = (((ULONGLONG)attributeHeader->Size + FILE_QUAD_ALIGNMENT) & ~FILE_QUAD_ALIGNMENT);
            ...
            attributeHeader = (PKSATTRIBUTE) (((PBYTE)attributeHeader) + (ULONG)cbAttribute);
            cbRemaining -= (ULONG)cbAttribute; // result is zero if end-of-buffer.
        }
    }
...
#pragma code_seg("PAGE")
NTSTATUS
CMiniportWaveRT::PropertyHandlerAudioLoopbackTappointCaps
(
    _In_ PPCPROPERTY_REQUEST      PropertyRequest
)
{
    ...
    DPF_ENTER(("[CMiniportWaveRT::PropertyHandlerAudioLoopbackTappointCaps]"));
    ...
    //
    // Validate pin # which is used as index in the format-modes array.
    //
    if (kspPin->PinId >= m_DeviceFormatsAndModesCount)
    {
        return STATUS_INVALID_PARAMETER;
    }
    ...
    if (PropertyRequest->Verb & KSPROPERTY_TYPE_GET)
    {
        ULONG * tappointCaps = (ULONG *)PropertyRequest->Value;

        *tappointCaps = (AUDIOLOOPBACK_TAPPOINT_CAPS_PREVOLUMEMUTE |
    ...
    return ntStatus;
} // PropertyHandlerAudioLoopbackTappointCaps
...
--*/
{
    ...
    else if (pWaveHelper->IsLoopbackTapPointSupported() &&
             (IsEqualGUIDAligned(*PropertyRequest->PropertyItem->Set, KSPROPSETID_AudioLoopback)))
    {
        switch(PropertyRequest->PropertyItem->Id)
        {
        case KSPROPERTY_AUDIOLOOPBACK_TAPPOINT_CAPS:
            ntStatus = pWaveHelper->PropertyHandlerAudioLoopbackTappointCaps(PropertyRequest);
            break;
